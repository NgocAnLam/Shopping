    ...
    if request.method == "POST":
        ...
        if password != confirm:
            return render_template(
                "admin/signup.html", error_message="Mật khẩu không trùng nhau"
            )

        if adminDB.find_one({"email": email}):
            return render_template(
                "admin/signup.html", error_message="Username already exists"
            )
...
def loginAdmin():
    ...
    if request.method == "POST":
        ...
        if admin and check_password_hash(admin["password"], password):
            ...
            adminDB.update_one(
                {"_id": admin["_id"]}, {"$set": {"loginTime": datetime.now()}}
            )
            return redirect(url_for("dashboardAdmin"))
        else:
            return render_template(
                "admin/login.html", error_message="Invalid username or password"
            )
...
def dashboardAdmin():
    ...
    countOrder = [
        sum(pair) for pair in zip(resultOrder[0]["result"], resultOrder[1]["result"])
    ]
...
@app.route("/admin/dashboard/chartRevenue", methods=["POST"])
def chartRevenueDay():
...
def get_a_year(year):
    ...
    result = {
        str(first_day + timedelta(days=i)): 0
...
def get_suggestion_remove():
    ...
    if query != "" and option == "id":
        ...
        result = productDB.find(
            {option: {"$type": "int", "$gte": input_int1, "$lte": input_int2}},
            {"_id": 0, option: 1},
        )
        return jsonify(
            {"success": True, "data": list(result), "query": query, "option": option}
        )

    elif query != "" and option == "name":
        result = productDB.find(
            {option: {"$regex": query, "$options": "i"}}, {"_id": 0, option: 1}
        )
        return jsonify(
            {"success": True, "data": list(result), "query": query, "option": option}
        )
        ...
        return jsonify({"success": True, "data": [], "query": query, "option": option})
...
def get_product_remove():
    ...
    return jsonify({"success": True, "data": result, "query": query, "option": option})
...
def update_product():
    ...
    result = productDB.update_one({"id": id}, {"$set": info})
...
def getCustomerList():
    ...
    if id == None and option == None:
        ...
        if option == "process":
            ...
            return render_template(
                "admin/customer.html", user=list(user), type="process"
            )
        elif option == "transit":
            user = accountDB.find({"ID": id}, {"_id": 0, "orderTransit": 1})
            return render_template(
                "admin/customer.html", user=list(user), type="transit"
            )
        elif option == "confirm":
            user = accountDB.find({"ID": id}, {"_id": 0, "orderList": 1})
            return render_template(
                "admin/customer.html", user=list(user), type="confirm"
            )
        else:
            user = accountDB.find({"ID": id}, {"_id": 0})
            return render_template("admin/customer.html", user=list(user), type="info")
...
def confirmOrderProcess(customerID, orderID):
    ...
    if document_to_transfer:
        ...
        accountDB.update_one(
            {"ID": customerID}, {"$push": {"orderTransit": target_order}}, upsert=True
        )
        ...
        return jsonify(
            {
                "success": True,
                "message": "Order added to the orderProcess successfully",
                "user": target_order,
            }
        )
    else:
        return jsonify(
            {
                "success": False,
                "message": "Order did not add to the orderProcess successfully",
            }
        )
...
def confirmOrderTransit(customerID, orderID):
    ...
    if document_to_transfer:
        ...
        accountDB.update_one(
            {"ID": customerID}, {"$push": {"orderList": target_order}}, upsert=True
        )
        ...
        return jsonify(
            {
                "success": True,
                "message": "Order added to the orderTransit successfully",
                "user": target_order,
            }
        )
    else:
        return jsonify(
            {
                "success": False,
                "message": "Order did not add to the orderTransit successfully",
            }
        )
...
def signup():
    if request.method == "POST":
        ...
        if password != confirm:
            return render_template(
                "signup.html", error_message="Mật khẩu không trùng nhau"
            )

        if accountDB.find_one({"email": email}):
            return render_template(
                "signup.html", error_message="Username already exists"
            )
...
def login():
    if request.method == "POST":
        ...
        if user and check_password_hash(user["password"], password):
            ...
            return render_template(
                "web/login.html", error_message="Invalid username or password"
            )
...
def category():
    categoryList = list(
        categoryDB.find({}, {"_id": 0, "text": 1, "name": 1, "code": 1, "icon_url": 1})
    )
...
def cart():
    ...
    if request.method == "POST":
        ...
        if existItem:
            ...
            if newQuantity == 0:
                ...
                return (
                    jsonify({"message": "Product removed from the cart successfully"}),
                    201,
                )
                ...
                if quantity > 0:
                    return (
                        jsonify({"message": "Product added to the cart successfully"}),
                        201,
                    )
                else:
                    return (
    ...
    if request.method == "DELETE":
        ...
        return (
            jsonify({"message": "All product removed from the cart successfully"}),
            201,
        )
...
def user(option):
    ...
    if request.method == "GET":
        ...
        if option == "watch":
            user = accountDB.find_one({"_id": ObjectId(userID)}, {"_id": 0})
            return render_template(
                "web/user.html", user=user, sideBar=sidebar, title="Sản phẩm đã xem"
            )
...
def buyItems():
    ...
    if request.method == "POST":
        ...
        return (
            jsonify({"message": "Products added to the orderProcessing successfully"}),
            201,
        )
